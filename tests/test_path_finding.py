...
class TestPathFindingConstructor(unittest.TestCase):
    ...
    def test___test(self):
        # setup
        import json

        with open("/home/dander/Documents/GitHub/Amira/test_data/amira_80_test_5/corrected_gene_calls_after_filtering.json") as i:
            calls = json.load(i)
        with open("/home/dander/Documents/GitHub/Amira/test_data/amira_80_test_5/corrected_gene_positions_after_filtering.json") as i:
            positions = json.load(i)
        graph = GeneMerGraph(calls, 5, positions)
        nodesOfInterest = []
        for geneOfInterest in ["catA1"]:
            nodesOfInterest += graph.get_nodes_containing(geneOfInterest)
        tree = construct_suffix_tree(graph.get_readNodes())
        calls = graph.get_reads().copy()
        rc_reads = {}
        ...
        finalAllelesOfInterest, copy_numbers = graph.split_into_subpaths(
            "catA1", full_blocks, full_block_coverages, 1
        )
        # assertion
        for f in finalAllelesOfInterest:
            print(f, finalAllelesOfInterest[f])
        ssss