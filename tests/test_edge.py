...
class TestEdgeConstructor(unittest.TestCase):
    ...
    def test___eq_edge_reads_identical(self):
        ...
        for n in range(len(nodes2) - 1):
            ...
            edges2.append(edge)
        # assertion
        self.assertTrue(all(edges2[i].__eq__(edges1[i]) and edges1[i].__eq__(edges2[i]) for i in range(len(edges1))))
    ...
    def test___hash_edge_same_complement_edges(self):
        ...
        edgesHashes2 = []
        for n in range(len(nodes2) - 1):
            edge = Edge(nodes2[n+1],
                        nodes2[n],
                        directions2[n+1],
                        directions2[n])
            edgesHashes2.append(edge.__hash__())
        # assertion
        self.assertTrue(all(nodes1[i].__hash__() == list(reversed(nodes2))[i].__hash__() for i in range(len(nodes1))))
        self.assertTrue(all(nodes2[i].__hash__() == list(reversed(nodes1))[i].__hash__() for i in range(len(nodes2))))
        self.assertTrue(all(edgesHashes2[i] == edgeHashes1[i] for i in range(len(edgeHashes1))))
        self.assertTrue(all(edgeHashes1[i] == edgesHashes2[i] for i in range(len(edgesHashes2))))
