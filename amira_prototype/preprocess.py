...
def parse_sam_file(sam_file_path, genesOfInterest):
    """
    ...
    for read in sam_file:
        if read.is_mapped:
            ...
            read_dict = {
                "sequence": sequence,
                "strand": strand,
                "identifier": read.reference_name.replace(".aln.fas", "").replace(".fasta", ""),
            }
    ...
    for readId in tqdm(read_amr_genes):
        ...
        for p in range(len(sorted_positions) - 1):
            if not sorted_positions[p][1] >= sorted_positions[p + 1][0]:
                new_positions += split_interval(
                    sorted_positions[p][1] + 1, sorted_positions[p + 1][0] - 1, interval_length
                )
        supplemented_read_data[readId] = {}
        for pos in new_positions:
            if pos:
                supplemented_read_data[readId][pos] = {
                    "sequence": fastq_content[readId]["sequence"][pos[0] : pos[1]]
                }
            # else:
...
def mmseqs2_easy_cluster(supplemented_read_data, threads, outputDir):
    ...
    subprocess.run(
        [
            "/home/daniel/Documents/GitHub/amira_prototype/panRG_building_tools/MMseqs2/build/bin/mmseqs",
            "easy-cluster",
            seq_file_path,
            cluster_file_path,
            "tmp",
...
def map_sequences_to_representatives(input_fasta, cluster_tsv, minimap2_output, threads, outputDir):
    ...
    minimap2_command = [
        ...
        "-o",
        minimap2_output,  # Output SAM file
    ]
        ...
        if query.is_mapped:
            if query.query_name in clustered[query.reference_name]:
                ...
                if not readId in clusterDict:
                    clusterDict[readId] = {}
                clusterDict[readId][(int(pos_start), int(pos_end))] = strand + str(
                    cluster_mapping[query.reference_name]
                )
    return clusterDict


def qc_readfile(readfile, filtered_readfile):
    filtlong_command = (
        "panRG_building_tools/Filtlong/bin/filtlong --min_length 1000 --keep_percent 95 "
        + readfile
        + " | gzip > "
        + filtered_readfile
    )
    subprocess.run(filtlong_command, shell=True, check=True)


def process_sam(samfile, readfile, outputDir, threads, genesOfInterest):
    read_amr_genes = parse_sam_file(samfile, genesOfInterest)
    filtered_readfile = os.path.join(
        os.path.dirname(readfile), os.path.basename(readfile).replace(".fastq", ".filtered.fastq")
    )
    qc_readfile(readfile, filtered_readfile)
    ...
    supplemented_read_data = supplement_read_data(read_amr_genes, fastq_content, 1000)
    mmseqs2_easy_cluster(supplemented_read_data, threads, outputDir)
    map_sequences_to_representatives(
        os.path.join(outputDir, "supplemented_read_data.fasta"),
        os.path.join(outputDir, "supplemented_read_data_cluster.tsv"),
        os.path.join(outputDir, "minimap2_output.sam"),
        threads,
        outputDir,
    )
    clusterDict = determine_cluster_strands(
        os.path.join(outputDir, "minimap2_output.sam"),
        os.path.join(outputDir, "supplemented_read_data_cluster.tsv"),
    )
    readGenes = {}
