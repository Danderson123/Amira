...
def map_reads(output_dir, reference_fasta, input_fasta, output_prefix, minimap_opts, minimap2_path):
        ...
        if promoter_name in reference_alleles:
            ...
            if "/" not in promoter_result["Identity (%)"]:
                if float(promoter_result["Identity (%)"]) == 100:
                    result_df = result_df.append(promoter_result, ignore_index=True)
            else:
